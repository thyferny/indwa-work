		...
		if(lines!=null){
				for(String[] vec:lines){
					if(vec.length>0)
					{
						...
						featureVec.add(0,  vec[this.classificationIndex].trim());
						String selectedCols = "";
						for(int j = 1; j < splits.length; j++){
							if(splits[j] != null) {
										selectedCols += colOrdered[j-1];
										selectedCols += ",";
							}
						}
						...
						boolean first= true;
						for(String dataValue:featureVec)
						{
							if(first)
							{
								realValue.append(dataValue);
								first=false;
							}
							else
							{
								realValue.append(AlpineHadoopConstants.SPECIAL_SEP_STRING).append(dataValue);
							}
							
						}
						keyValue.set(node + "\t" + selectedCols);
						dataValue.set(realValue.toString());
