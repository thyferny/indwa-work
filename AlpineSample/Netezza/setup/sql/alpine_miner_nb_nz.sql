...
nominal_columns_table  ALIAS FOR $3;
nominal_mapping_count_table  ALIAS FOR $4;
nominal_columns_mapping_table  ALIAS FOR $5;
nominal_columns_pro_table  ALIAS FOR $6;
dependent_column_arg  ALIAS FOR $7;
dependent_column_mapping_table  ALIAS FOR $8;
dependent_column_pro_table  ALIAS FOR $9;
numerical_columns_table  ALIAS FOR $10;
numerical_columns_pro_table  ALIAS FOR $11;


nominal_columns_count int := 0;
nominal_columns_name_array varray(2000) of varchar(2000);
nominal_mapping_count_count int := 0;
nominal_mapping_count_array varray(2000) of int;
nominal_columns_mapping_count int := 0;
nominal_columns_mapping_array varray(2000) of varchar(2000);

nominal_columns_pro_count int := 0;
nominal_columns_pro_array varray(2000) of double;

dependent_column_mapping_count int := 0;
dependent_column_mapping_array varray(2000) of varchar(2000);

dependent_column_pro_count int := 0;
dependent_column_pro_array varray(2000) of double;

numerical_columns_count int := 0;
numerical_columns_name_array varray(2000) of varchar(2000);

numerical_columns_pro_count int := 0;
numerical_columns_pro_array varray(2000) of double;

column_value_array varray(2000) of varchar(2000);

numerical_column_value_array varray(2000) of double;
...
temp_varchar varchar(200);
temp_varchar1 varchar(200);
temp_varchar2 varchar(200);
temp_varchar3 varchar(200);
 

begin
        i := 0;
        mysql := 'select value from '||nominal_columns_table||' order by id ';
        for myrecord in execute mysql loop
                nominal_columns_count := nominal_columns_count + 1;
                nominal_columns_name_array(nominal_columns_count) := myrecord.value;
        end loop;


        mysql := 'select value from '||nominal_mapping_count_table||' order by id ';
        for myrecord in execute mysql loop
                nominal_mapping_count_count := nominal_mapping_count_count + 1;
                nominal_mapping_count_array(nominal_mapping_count_count) := myrecord.value;
        end loop;


        mysql := 'select value from '||nominal_columns_mapping_table||' order by id ';
        for myrecord in execute mysql loop
                nominal_columns_mapping_count := nominal_columns_mapping_count + 1;
                nominal_columns_mapping_array(nominal_columns_mapping_count) := myrecord.value;
        end loop;

        mysql := 'select value from '||nominal_columns_pro_table||' order by id ';
        for myrecord in execute mysql loop
                nominal_columns_pro_count := nominal_columns_pro_count + 1;
                nominal_columns_pro_array(nominal_columns_pro_count) := myrecord.value;
        end loop;
        ...
        mysql := 'select value from '||dependent_column_pro_table||' order by id ';
        for myrecord in execute mysql loop
                dependent_column_pro_count := dependent_column_pro_count + 1;
                dependent_column_pro_array(dependent_column_pro_count) := myrecord.value;
        end loop;

        mysql := 'select value from '||numerical_columns_table||' order by id ';
        for myrecord in execute mysql loop
                numerical_columns_count := numerical_columns_count + 1;
                numerical_columns_name_array(numerical_columns_count) := myrecord.value;
        end loop;

        mysql := 'select value from '||numerical_columns_pro_table||' order by id ';
        for myrecord in execute mysql loop
                numerical_columns_pro_count := numerical_columns_pro_count + 1;
                numerical_columns_pro_array(numerical_columns_pro_count) := myrecord.value;
        end loop;


	i = 1;
	while( i <= numerical_columns_count) loop
        	nominal_columns_name_array(nominal_columns_count + i) := numerical_columns_name_array(i);
        	i = i + 1;
	end loop;

	nominal_columns_name_array(nominal_columns_count + numerical_columns_count + 1) = dependent_column_arg;
	all_count := nominal_columns_count + numerical_columns_count + 1;

        mysql := 'select ';
        for i in 1.. all_count loop
                if i <> 1 then
                        mysql := mysql||',';
                end if;
                tempstr := nominal_columns_name_array(i);
                mysql := mysql||tempstr;
                mysql := mysql||' as c';
                mysql := mysql||i;
        end loop;
	...
	if all_count >= 1600 then
	 column_value_array(1600) := myrecord.c1600;
        ...
        j := 1;
        while( j <= numerical_columns_count) loop
	     temp_varchar1 := column_value_array(nominal_columns_count + j);
             numerical_column_value_array(j) := temp_varchar1::double;
             j := j + 1;
        end loop;
	temp_varchar := column_value_array(nominal_columns_count + numerical_columns_count + 1);
	dependent_column_value := temp_varchar;



        if(nominal_columns_count > 0 and nominal_mapping_count_count > 0 and nominal_columns_mapping_count > 0 and nominal_columns_pro_count > 0)
        then
                nominal_null := 0;
        end if;

        if (numerical_columns_count > 0 and numerical_columns_pro_count > 0)
        then
                numerical_null := 0;
        end if;

        for i  in  1 .. dependent_column_mapping_count loop
		temp_double := dependent_column_pro_array(i);
                pro(i) := temp_double;
        end loop;
        if  nominal_null = 0
        then
                for i in 1 .. dependent_column_mapping_count loop
                        ...
                        for j in 1.. nominal_columns_count loop
				temp_int1 := nominal_mapping_count_array(j);
                                pro_index := pro_column_offset + (i - 1)* temp_int1; 
                                for k in  1.. temp_int1 loop
					temp_varchar1 := column_value_array(j);
					temp_varchar2 := nominal_columns_mapping_array(nominal_mapping_index + k );
                                        if (temp_varchar1 = temp_varchar2)
                                        then
						temp_double1 := pro(i);
						temp_double2 := nominal_columns_pro_array(pro_index + 1);
                                                pro(i) :=  temp_double1 + temp_double2;
                                                EXIT ; 
                                        end if;
                                        pro_index := pro_index + 1;
                                end loop;
                                pro_column_offset := pro_column_offset + dependent_column_mapping_count * temp_int1;
                                nominal_mapping_index := nominal_mapping_index + temp_int1;
                        end loop;
                end loop;
        end if;
        if (numerical_null = 0) then
                for i in 1.. dependent_column_mapping_count loop
                        for j in 1.. numerical_columns_count loop
                                pro_index := numerical_pro_length * (j - 1) * dependent_column_mapping_count;
                                pro_index := pro_index + numerical_pro_length * (i - 1) + 1;
				temp_double1 := numerical_column_value_array(j);
				temp_double2 := numerical_columns_pro_array(pro_index);
				temp_double3 := numerical_columns_pro_array(pro_index + 1);
                                base := (temp_double1 - temp_double2)
                                        /temp_double3;
				temp_double1 := pro(i) ;
				temp_double2 := numerical_columns_pro_array(pro_index + 2);
                                pro(i) := temp_double1 - (temp_double2 + 0.5 * base * base);
                        end loop;
                end loop;
        end if;
	...
	max_log_pro := 0;
        for i in 1.. dependent_column_mapping_count loop
		temp_double1 = pro(i);
                if i = 1 or max_log_pro  < temp_double1 then
                        max_log_pro := temp_double1;
                end if;
        end loop;

        for i in 1.. dependent_column_mapping_count loop
		temp_double1 = pro(i);
                diff := temp_double1 - max_log_pro;
                if diff < -45 then
                        pro(i) := 0.0000001;
                else
                        pro(i) := exp(diff);
	...
	pro_sum := 0;
        for i in 1.. dependent_column_mapping_count loop
		temp_double1 = pro(i);
                pro_sum := pro_sum + temp_double1;
        end loop;

        for i in 1 .. dependent_column_mapping_count loop
		temp_double1 = pro(i);
                pro(i) := temp_double1/pro_sum;
        end loop; 

        for i in 1.. dependent_column_mapping_count loop
	temp_varchar1 :=  dependent_column_mapping_array(i);
	deviance := 0;
        if (dependent_column_value = temp_varchar1)
                then
			temp_double1 = pro(i);
                        deviance := -2.0*ln(temp_double1);
                        exit;
                end if;
        end loop;
	if column_index = 1 then
        	deviance_sum := deviance;
	else
		deviance_sum := deviance_sum + deviance;
	end if;
...
create or replace procedure alpine_miner_nb_ca_predict_proc(
varchar(64000),
varchar(64000),
varchar(64000),
varchar(64000),
...
nominal_columns_mapping_table  ALIAS FOR $5;
nominal_columns_pro_table  ALIAS FOR $6;
dependent_column_pro_table  ALIAS FOR $7;
numerical_columns_table  ALIAS FOR $8;
numerical_columns_pro_table  ALIAS FOR $9;
result_table  ALIAS FOR $10;
...
begin
	...
	nominal_columns_name_array(nominal_columns_count + numerical_columns_count + 1) = 'alpine_miner_id';
	all_count := nominal_columns_count + numerical_columns_count + 1;
	...
	id := temp_varchar::bigint;
        ...
        for i  in  1 .. dependent_column_pro_count loop
		temp_double := dependent_column_pro_array(i);
                pro(i) := temp_double;
        end loop;
        if  nominal_null = 0
				...
				temp_int1 := nominal_mapping_count_array(j);
					...
					temp_varchar2 := nominal_columns_mapping_array(nominal_mapping_index + k );
						...
						temp_double2 := nominal_columns_pro_array(pro_index + 1);
                                ...
                                pro_column_offset := pro_column_offset + dependent_column_pro_count * temp_int1;
                                nominal_mapping_index := nominal_mapping_index + temp_int1;
                        end loop;
                end loop;
        end if;
        if (numerical_null = 0) then
                for i in 1.. dependent_column_pro_count loop
                        for j in 1.. numerical_columns_count loop
                                pro_index := numerical_pro_length * (j - 1) * dependent_column_pro_count;
                                pro_index := pro_index + numerical_pro_length * (i - 1) + 1;
				temp_double1 := numerical_column_value_array(j);
				temp_double2 := numerical_columns_pro_array(pro_index);
				temp_double3 := numerical_columns_pro_array(pro_index + 1);
