...
create or replace procedure alpine_miner_lir_ca_predict_proc(
varchar(64000),
varchar(64000),
varchar(64000),
varchar(64000),
...
result_table  ALIAS FOR $5;
result  double := 0;
beta_array  varray(2000) of double;
beta_count int := 0;
columns_count int := 0;
columns_name_array varray(2000) of varchar(2000);
...
begin
        ...
        for myrecord in execute mysql loop
                ...
                columns_name_array(columns_count) := myrecord.value;
        end loop;

		columns_name_array(columns_count + 1) = 'alpine_miner_id';
	...
	id := column_value_array(columns_count + 1);

	if(beta_count < columns_count + 1) then
		temp_int := beta_count;
	else
		temp_int := columns_count + 1;
	end if;
	result := 0;
	for i in 1.. (temp_int -1) loop
		temp_double1 := beta_array(i);
		temp_double2 := column_value_array(i);
		result := result + temp_double1 * temp_double2;
	end loop;
	result := result + beta_array(beta_count);

	mysql := 'insert into '||result_table||' values ('||id||','||result || ' ) ';;
...
create or replace procedure alpine_miner_lir_ca_r2s_proc(
varchar(64000),
varchar(64000),
varchar(64000),
double,
...
where_condition  ALIAS FOR $2;
label  ALIAS FOR $3;
label_avg  ALIAS FOR $4;
dof  ALIAS FOR $5;
beta_table  ALIAS FOR $6;
columns_table  ALIAS FOR $7;
...
begin
		...
		columns_name_array(columns_count + 1) = label;
		all_count := columns_count + 1;
	...
	label_value := column_value_array(columns_count + 1);
	...
	result := result + beta_array(beta_count);
  	...
  	SSerror := SSerror + (result - label_value) * (result - label_value); 
  	SStotal := SStotal + (label_value - label_avg) * (label_value - label_avg); 

	end loop;

       if SStotal <> 0 then
		r := 1.0 - SSerror/SStotal;
       else
		r := 0;
       end if;
       if (dof > 0) then
	...
	mysql := 'insert into '||result_table||' values (1,'||r || ' ) ';;
	execute immediate mysql;

	mysql := 'insert into '||result_table||' values (2,'||s || ' ) ';;
	execute immediate mysql;
