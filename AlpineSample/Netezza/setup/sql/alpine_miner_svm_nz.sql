create or replace
procedure alpine_miner_online_sv_cl(
varchar(64000),
varchar(64000),
varchar(64000),
varchar(64000), 
...
column_table ALIAS FOR $2;
label ALIAS FOR $3;
wherecond_table ALIAS FOR $4;
kernel_type ALIAS FOR $5;
degree ALIAS FOR $6; 
gamma ALIAS FOR $7;
eta ALIAS FOR $8;
nu ALIAS FOR $9;
individuals_table ALIAS FOR $10;
weights_table ALIAS FOR $11;
model_table ALIAS FOR $12;
temp_int int := 0;
temp_double1 double := 0;
temp_double2 double := 0;
p double;
sqlstr varchar(64000);
labelvalue double;
i int := 0;
j int := 0;
k int := 0;
wherestr varchar(64000):= '';

kernel_ret double := 0;
individual_array varray(2000) of double;

len int;
ret double := 0;
temp double;
diff  varray(2000) of double;

column_count int := 0;
wherecond_array varray(2000) of varchar(200);
myrecord record;
myrecord1 record;
mysql varchar(64000);
wherecond_count int := 0;
loop_count int := 0;

column_name_array varray(2000) of varchar(200);
...
column_index int := 0;
all_count int := 0;
tempstr varchar(20000);
myrecord_individual record;
myrecord_weights record;
weight double;
msvs_sql varchar(64000);
BEGIN
  ...
  k := 0;
        ...
        mysql := 'select value from '||column_table||' order by id ';
         column_count := 0;
        for myrecord in execute mysql loop
                column_count := column_count + 1;
                column_name_array(column_count) := myrecord.value;
        end loop;

        mysql := 'select value from '||wherecond_table||' order by id ';
	wherecond_count := 0;
        for myrecord in execute mysql loop
                wherecond_count := wherecond_count + 1;
                wherecond_array(wherecond_count) := myrecord.value;
        end loop;
	for i in 1.. wherecond_count loop
	      if (i != 1) then
	              wherestr := wherestr||' and ';
	      end if;
		
              tempstr := wherecond_array(i);
	      wherestr := wherestr||tempstr||' is not null ';
	end loop;
  	wherestr := wherestr||' and '||label||' is not null ';
        ...
        for i in 1.. column_count loop
                ...
                tempstr := column_name_array(i);
                mysql := mysql||tempstr;
                mysql := mysql||' as c';
                mysql := mysql||i;
        end loop;
	all_count  := column_count + 1;
	mysql := mysql||','||label||' as c'||all_count;
        mysql := mysql || ' from '||table_name||' where '||wherestr;
        i := 0;
	column_index := 0;
  for myrecord in execute mysql loop
	...
	labelvalue := column_value_array(all_count);
  kernel_ret := 0;
    ret := 0;

  IF j = 0 THEN
  ind_dim := column_count;
  END IF;


	mysql := 'select value from ' || individuals_table || ' order by id ';
	
	col_count := 0;
	row_count := 0;

	loop_count := 0;
    for myrecord_individual in execute mysql loop

	    loop_count := loop_count + 1;
   if ((loop_count % column_count) = 0) then
		individual_array(column_count) := myrecord_individual.value;
	    kernel_ret := 0;
	if kernel_type = 1 then
		if individual_count < column_count then
			len := individual_count;
		else
			len := column_count;
		end if;
		FOR i in 1.. len LOOP
			temp_double1 := individual_array(i);
			temp_double2 := column_value_array(i);
			kernel_ret := kernel_ret + temp_double1 * temp_double2;
		END LOOP;
	else
		if kernel_type = 2 then
			...
			if kernel_type = 3 then
				...
				for i in 1.. len loop
					diff(i) := individual_array(i) - column_value_array(i);
				end loop;
				...
				FOR i in 1.. len LOOP
		                        temp_double1 := diff(i);
		                        kernel_ret := kernel_ret + temp_double1 * temp_double1;
				END LOOP;
	
				temp := -1.0 * gamma * kernel_ret;
				if (temp < -30) then
					temp := -30;
				elsif(temp > 30) then
					temp := 30;
	...
	msvs_sql := 'select value from ' || weights_table || ' where  id = '||i;
	for myrecord_weights in execute msvs_sql loop
	 	weight := myrecord_weights.value;
	end loop;
	ret := ret + weight * kernel_ret;
     else
		individual_array(loop_count % column_count) := myrecord_individual.value;  	
     end if;
   END LOOP;
	...
	IF (p <= rho) THEN
		temp_double1 := (1 - 0.1 * eta);
		mysql := 'update ' || weights_table ||' set value = case when abs(value) < 1.15 * 2.2250738585072014e-308::double then 0 else value *('||temp_double1||')::double end ';
		  execute immediate mysql ;
		    nsvs := nsvs + 1;
		weight := labelvalue * eta;
		mysql := 'insert into '|| weights_table ||' values('|| nsvs || ' ,' || weight ||')';
		execute immediate mysql;
		FOR i IN 1.. column_count LOOP
		  temp_double1 := column_value_array(i);
		  temp_int := (nsvs - 1)*column_count+i;
		  mysql := 'insert into '|| individuals_table ||' values('|| temp_int || ' ,'|| temp_double1 ||')';
		  execute immediate mysql;
		END LOOP;
	    	b := b + eta * labelvalue;
		rho := rho - eta * (1 - nu);
  ...
  execute immediate 'insert into '||model_table||' values(1, '||inds||')';
  execute immediate 'insert into '||model_table||' values(2, '||cum_err||')';
  execute immediate 'insert into '||model_table||' values(3, '||epsilon||')';
  execute immediate 'insert into '||model_table||' values(4, '||rho||')';
  execute immediate 'insert into '||model_table||' values(5, '||b||')';
  execute immediate 'insert into '||model_table||' values(6, '||nsvs||')';
  execute immediate 'insert into '||model_table||' values(7, '||ind_dim||')';
...
create or replace
procedure alpine_miner_online_sv_reg(
varchar(64000),
varchar(64000),
varchar(64000),
varchar(64000), 
...
slambda ALIAS FOR $9;
nu ALIAS FOR $10;
individuals_table ALIAS FOR $11;
weights_table ALIAS FOR $12;
model_table ALIAS FOR $13;
...
ret double := 0;
temp double;
diff_array  varray(2000) of double;
diff double;
...
BEGIN
	...
	if kernel_type = 1 then
		...
		if kernel_type = 2 then
			...
			if kernel_type = 3 then
				...
				for i in 1.. len loop
					diff_array(i) := individual_array(i) - column_value_array(i);
				end loop;
    ...
    IF (error > epsilon) THEN
		...
		temp_double1 := 1 - eta * slambda;
		mysql := 'update ' || weights_table ||' set value = case when abs(value) < ('||cap||' + 0.1) * 2.2250738585072014e-308::double then 0 else value *('||temp_double1||')::double end ';
		  execute immediate mysql ;
		    nsvs := nsvs + 1;
        weight := eta;
        IF (diff < 0)
...
create or replace
procedure alpine_miner_online_sv_nd(
varchar(64000),
varchar(64000),
varchar(64000), 
int, 
...
table_name ALIAS FOR $1;
column_table ALIAS FOR $2;
wherecond_table ALIAS FOR $3;
kernel_type ALIAS FOR $4;
degree ALIAS FOR $5; 
gamma ALIAS FOR $6;
eta ALIAS FOR $7;
nu ALIAS FOR $8;
individuals_table ALIAS FOR $9;
weights_table ALIAS FOR $10;
model_table ALIAS FOR $11;
temp_int int := 0;
temp_double1 double := 0;
temp_double2 double := 0;
p double;
...
epsilon double := 0;
rho double := 0;
b   double := 0;--- not the same with cl reg nd
nsvs int := 0;
ind_dim int := 0;
...
BEGIN
	...
	IF (p < rho) THEN
		...
		temp_double1 := (1 - 0.1 * eta);
		mysql := 'update ' || weights_table ||' set value = case when value < 1.15 * 2.2250738585072014e-308::double then 0 else value *('||temp_double1||')::double end ';
		  execute immediate mysql ;
		    nsvs := nsvs + 1;
		weight := eta;
		mysql := 'insert into '|| weights_table ||' values('|| nsvs || ' ,' || weight ||')';
...
degree ALIAS FOR $5; 
gamma ALIAS FOR $6;
individuals_table ALIAS FOR $7;
weights_table ALIAS FOR $8;
temp_table_name ALIAS FOR $9;
...
BEGIN
	...
	id :=  column_value_array(all_count);
	kernel_ret := 0;
	ret := 0;
   ...
   END LOOP;
   mysql = ' insert into '||temp_table_name||' values ( '||id||','||ret||')';
   execute immediate  mysql; 
   j := j + 1;
   END LOOP;
   return 0;
