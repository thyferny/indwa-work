create or replace procedure alpine_miner_lr_ca_beta_proc(
varchar(64000), 
varchar(64000),
varchar(64000) ,
varchar(64000),
...
beta_table ALIAS FOR $3;
column_table ALIAS FOR $4;
add_intercept ALIAS FOR $5;
y_name ALIAS FOR $6;
times ALIAS FOR $7;
result_table ALIAS FOR $8;

y DOUBLE default 1;
weight DOUBLE default 1.0;
beta_sql varchar(64000) default '';
tempstr varchar(1000) ;
column_value_array VARRAY(2000) OF DOUBLE;
beta_array VARRAY(2000) OF DOUBLE;
column_name_array VARRAY(2000) OF varchar(64000);
mysql varchar(64000);
myrecord record;
row_result_length int;
eta double := 0;
exp_eta double := 0;
mu_eta_dev double := 0;
foo double := 0;
result_data VARRAY(1290000) OF DOUBLE;
result_sum_data VARRAY(1290000) OF DOUBLE;
result_data_xwz VARRAY(1290000) OF DOUBLE;
...
begin
        ...
        mysql := 'select value from '||beta_table||' order by id ';
        beta_count := 0;
        for myrecord in execute mysql loop
                beta_count := beta_count + 1;
                beta_array(beta_count) := myrecord.value;
        end loop;
        ...
        mysql := mysql ||' , '|| y_name || ' as c'||(column_count+1);
        mysql := mysql || ' from '||table_name||' '||where_condition;
        i := 0;
	...
	y := column_value_array(all_count);
	...
	while (i <= beta_count and i <= column_count) loop
		gx := gx + (beta_array(i)) * (column_value_array(i));
		i := i + 1;
	end loop;
	if add_intercept = 1 then
		gx := gx + (beta_array(beta_count));
	end if;
	...
	while (i <= beta_count) loop
		if ((i = (beta_count)) and add_intercept = 1) then
			x := 1.0;
		else
			x := (column_value_array(i));
		end if;
		j := i;
		while(j <= beta_count) loop
			if ((j = (beta_count)) and add_intercept = 1) then
				ca_y := 1.0;
			else
				ca_y := (column_value_array(j));
			end if;
			result_data(ind) := (-x*ca_y*weight*pi*(1.0 - pi));
			ind := ind + 1;
			j := j + 1;
		end loop;
		i := i + 1;
	...
	foo := weight * pi*(1-pi)*(eta+(y - pi)/mu_eta_dev);
	while (i <= column_count) loop
		result_data_xwz(i) := (column_value_array(i))*foo;
		i := i + 1;
	end loop;
	if (add_intercept = 1) then
		result_data_xwz(column_count + 1) := foo;
	end if;
  ...
  if add_intercept = 1 then
	...
	if column_index = 1 then
		result_sum_data(i) := result_data(i) ;
	else
		result_sum_data(i) := result_data(i) + result_sum_data(i);
	end if;
	end loop;
	end loop;
	for i in 1.. result_count loop
		tmp := result_sum_data(i) ;
	        execute immediate 'insert into '||result_table||' values('||i||','||tmp||')';
	end loop;
--	return result_data;
	return 1;
...
create or replace procedure alpine_miner_lr_ca_fitness_proc(
varchar(64000),
varchar(64000),
varchar(64000) ,
varchar(64000),
...
y_name ALIAS FOR $6;
 ...
 y double default 1;--
 weight double default 1.0;
 mysql VARCHAR(32672) DEFAULT '';
 tempstr VARCHAR(32672) DEFAULT '';
 beta_sql varchar(32672) default '';
column_value_array VARRAY(2000) OF DOUBLE;
beta_array VARRAY(2000) OF DOUBLE;
column_name_array VARRAY(2000) OF varchar(64000);
myrecord record;
 row_result double default 0;
  i int := 1;
	j int := 1;
	ind int := 1;
	fitness double := 0.0;
...
create or replace procedure alpine_miner_lr_ca_he_proc(
...
column_name_array VARRAY(2000) OF varchar(64000);
 row_result double default 0;
  i int := 1;
	...
	result_array varray(1290000) of double;
	result_sum_array varray(1290000) of double;
	...
	while (i <= beta_count) loop
		...
		while(j <= beta_count) loop
			if ((j = (beta_count)) and add_intercept = 1) then
				...
				y := (column_value_array(j));
			end if;
			result_array(ind) := (-x*y*weight*pi*(1.0 - pi));
			if(column_index = 1) then
				result_sum_array(ind) := result_array(ind) ;
			else
				result_sum_array(ind) := result_array(ind) + result_sum_array(ind);	
			end if;
			y := result_sum_array(ind);
			ind := ind + 1;
			j := j + 1;
		end loop;
		i := i + 1;
...
create or replace procedure alpine_miner_lr_ca_he_de_proc(
...
add_intercept ALIAS FOR $5;
y_name ALIAS FOR $6;
result_table ALIAS FOR $7;



        ca_y double := 0;
  result_data_der varray(2000) of double;
...
begin
	...
	foo := weight * (y - pi);
	while (i <= column_count) loop
		result_data_der(i) := (column_value_array(i))*foo;
		i := i + 1;
	end loop;
	if (add_intercept = 1) then
		result_data_der(column_count + 1) := foo;
	end if;
  ...
  if add_intercept = 1 then
	...
	for i in 1.. result_count loop
		...
		x := result_data(i); y := result_sum_data(i);
	end loop;
...
create or replace procedure alpine_miner_lr_ca_predict_proc(
...
begin
	...
	gx := 0;
	while (i <= beta_count and i <= column_count) loop
		temp_double1 := beta_array(i);
		temp_double2 := column_value_array(i);
		gx := gx + temp_double1 * temp_double2;
		i := i + 1;
	end loop;
	if add_intercept = 1 then
		temp_double1 := beta_array(beta_count);
		gx := gx + temp_double1;
	end if;
	...
	pi := tmp/(1.0 + tmp);
        execute immediate 'insert into '||result_table||' values('||column_index||','||pi||')';
