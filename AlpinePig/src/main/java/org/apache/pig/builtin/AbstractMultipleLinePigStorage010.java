...
public abstract class AbstractMultipleLinePigStorage010   extends AbstractPigStorage010 {
   ...
   protected List<Tuple> bufferResult= new ArrayList<Tuple>(); 	    
    ...
    public AbstractMultipleLinePigStorage010(RecordParser recordParser,String typeString,String callBackURL ,String uuid, String operatorName){
		super(typeString,callBackURL, uuid,  operatorName); //xpathListString is separatered with ,
...
//				 boolean badDataFound= false;
					 ...
					 for (int fieldIndex = 0; fieldIndex < values.length; fieldIndex++) {
						 //for the projection, make sure filter the columns pig want!!
						 if (mRequiredColumns==null || (mRequiredColumns.length>fieldIndex && mRequiredColumns[fieldIndex])){
							 if (values[fieldIndex] == null||values[fieldIndex].length()==0) {
									//null value
...
//									badDataFound=true;
						    		...
						    		mProtoTuple.add(new DataByteArray( values[fieldIndex].getBytes("UTF-8")));
						    	}
...
//					 }
						 ...
						 resutl.add( dontLoadSchema ? t : applySchema(t) ); 
