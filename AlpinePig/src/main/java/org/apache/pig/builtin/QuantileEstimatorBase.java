 ...
 *
 */
public abstract class QuantileEstimatorBase {
	...
	private double min;
	private double max;
	private double totalValueOfElements = 0.0;
	...
	protected QuantileEstimatorBase(int numQuantiles)
	{
	...
	public void add(double elem)
	{
		...
		totalElements++;
		totalValueOfElements += elem;
	}

	public void clear() {
		buffer.clear();
		totalElements = 0;
		totalValueOfElements = 0;
	}
	...
	public List<Double> getQuantiles()
	{
		...
		if (buffer.size() > 1 && buffer.get(1) != null) {
			Collections.sort(buffer.get(1));
		}
		...
		for (int i = 1; i <= numQuantiles - 2; i++) {
			long targetS = (long) Math.floor(i * (totalElements / (numQuantiles - 1.0))) + 1;  //changed to match database  (used to be ceiling without the +1)
	...
	public double getMean() {
		double mean = 0.0;
		if (totalElements != 0)
			mean =  totalValueOfElements /  totalElements;
		return mean;
	}

	public double getTotalValueOfElements() {
		return totalValueOfElements;
	}
}
