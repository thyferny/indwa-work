/**
 * ClassName DecisionTreeOperator.java
 *
 ...
 *
 */
public class RandomForestOperator extends LearnerOperator {
...
//			OperatorParameter.NAME_size_threshold_load_data,
			...
			OperatorParameter.NAME_forestSize,
			OperatorParameter.NAME_nodeColumnNumber,
			OperatorParameter.NAME_sample_with_replacement ,
			
			
	});

	public RandomForestOperator() {
		super(parameterNames);
		addInputClass(OperatorInputTableInfo.class.getName());
		addOutputClass(EngineModel.MPDE_TYPE_TREE_RANDOM_FOREST);
		addOutputClass(RandomForestModelDB2.class.getName());
		addOutputClass(RandomForestModelNZ.class.getName());

		addOutputClass(RandomForestModelOracle.class.getName());

		addOutputClass(RandomForestModelGreenplum.class.getName());
	...
	public boolean isVaild(VariableModel variableModel) {
		...
		for(OperatorParameter para:paraList){
			...
			}else if(paraName.equals(OperatorParameter.NAME_forestSize)){
				validateNull(invalidParameterList, paraName, paraValue);
				validateInteger(invalidParameterList, paraName, paraValue,1,false,Integer.MAX_VALUE,true,variableModel);
			}
			else if(paraName.equals(OperatorParameter.NAME_nodeColumnNumber)){
				validateNull(invalidParameterList, paraName, paraValue);
				validateInteger(invalidParameterList, paraName, paraValue,1,false,Integer.MAX_VALUE,true,variableModel);
	...
	public String getOperatorParameterDefaultValue(String paraName) {
		...
		else if(paraName.equals(OperatorParameter.NAME_sample_with_replacement)){
			return Resources.FalseOpt;
		}
		else{
