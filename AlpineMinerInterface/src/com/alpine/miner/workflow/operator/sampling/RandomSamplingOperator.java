 ...
 *
 */
public class RandomSamplingOperator extends AbstractSamplingOperator {
	...
	public static final List<String> parameterNames = Arrays.asList(new String[]{
			...
			OperatorParameter.NAME_replacement,
	...
	public boolean isVaild(VariableModel variableModel) {
		...
		for(OperatorParameter para:paraList){
			...
			if(para.getValue() instanceof String){
				...
				}else if(paraName.equals(OperatorParameter.NAME_replacement)){
		...
		for(OperatorParameter para:paraList){
			...
			if(paraName.equals(OperatorParameter.NAME_sampleCount)){
				...
				if(sampleSizeModel==null
					...
					if((StringUtil.isEmpty(replacement)==false&&replacement.equals(com.alpine.utility.db.Resources.FalseOpt))
							||StringUtil.isEmpty(replacement)==true){
					...
					for(int i=0;i<sampleSizeModel.getSampleIdList().size();i++){
						...
						if(!StringUtil.isEmpty(sampleSizeType)){
							if(sampleSizeType
								...
								if(isReplacement){
									validateNumber(invalidParameterList, paraName, sampleSize.toString(),0,true,Double.MAX_VALUE,false,variableModel);
								}else{
				...
				if(!StringUtil.isEmpty(consistent)
						...
						&&(StringUtil.isEmpty(replacement)||replacement.equalsIgnoreCase("false"))){
					validateNull(invalidParameterList, paraName, paraValue);
	...
	public String getOperatorParameterDefaultValue(String paraName) {
		if(paraName.equals(OperatorParameter.NAME_replacement)){
