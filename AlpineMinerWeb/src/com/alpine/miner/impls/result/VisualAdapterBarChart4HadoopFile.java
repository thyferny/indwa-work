...
public class VisualAdapterBarChart4HadoopFile extends AbstractOutPutVisualAdapter  implements OutPutVisualAdapter {
	
	public static final OutPutVisualAdapter INSTANCE = new VisualAdapterBarChart4HadoopFile();
...
//        }
        ...
        /*
        - MINERWEB-1148
        - This sampling of data ends up producing a chart that is not very useful because the bars shown for each category can be out of sync
        - It's very situational what is shown/hidden
        - We should decide a universal way of handling this situation if we don't want to shown more than x bars
        - TODO: decide how to handle this: temporarily hardcode to 100 and show warning when > 100
        */
        maxCount=100;
        if(total>maxCount){
...
//        }
	...
	private List<String> getValueList(DataTable dataTable,
		...
		if(rows!=null){
			for (Iterator<DataRow> iterator = rows.iterator(); iterator.hasNext();) {
				...
				if(null!=dataRow.getData(index) && values.contains(dataRow.getData(index))==false){
	...
	private String findValue(String cateValue, String scopeValue,
		...
		if(rows!=null){
			for (Iterator<DataRow> iterator = rows.iterator(); iterator.hasNext();) {
				DataRow dataRow = iterator.next();
				if(cateValue!=null && scopeValue.equals(dataRow.getData(scopeIndex))
