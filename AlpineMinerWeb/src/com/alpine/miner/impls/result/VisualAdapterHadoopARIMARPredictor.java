...
public class VisualAdapterHadoopARIMARPredictor extends VisualAdapterBaseARIMARPredictor
	...
	public VisualizationModel toVisualModel(AnalyticOutPut analyzerOutPut,Locale locale)
		...
		if(!(analyzerOutPut instanceof HadoopAnalyzerOutPutARIMARPredict)){
			return null;
		}

        HadoopAnalyzerOutPutARIMARPredict model=(HadoopAnalyzerOutPutARIMARPredict)analyzerOutPut;
		HadoopARIMARPredictResult aResult=model.getRet();
		int maxLines=OutPutVisualAdapterFactory.getInstance().getMaxChartElements();
		List<String> errorMessages = new ArrayList<String>();
		List<HadoopSingleARIMARPredictResult> aRsults = aResult.getResults();
	...
	private void addTableResult(		List<HadoopSingleARIMARPredictResult> results,
		...
		if(results!=null&&results.size()>0){
			...
			if(StringUtil.isEmpty(groupByColumnName)){
                HadoopSingleARIMARPredictResult aRresult = (HadoopSingleARIMARPredictResult) results.get(0);
				...
				for (Iterator iterator = results.iterator(); iterator.hasNext();) {
                    HadoopSingleARIMARPredictResult singleARIMARPredictResult = (HadoopSingleARIMARPredictResult) iterator
							.next();
	...
	private DataTable getTableOutPut(HadoopSingleARIMARPredictResult aResult) {
	...
	private VisualizationModelLine createLineModels(List<HadoopSingleARIMARPredictResult> aResults ,Locale locale) {
		...
		lineModel.setxAxisTitle(aResults.get(0).getIdColumn()+precisionTitles[0]);
	...
	private List<String[]> addLinesForSingleResult(List<VisualLine> lines,
             HadoopSingleARIMARPredictResult aResult,int index,Locale locale,List<float[]> precisionList) {
		List<String[]> xLabels = new ArrayList<String[]> ();
		List<String[]> yLabels = new ArrayList<String[]> ();

		String idAttribute = aResult.getIdColumn();
