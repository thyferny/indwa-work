...
public class VisualAdapterLinearRegression extends AbstractOutPutVisualAdapter implements OutPutVisualAdapter {
	 

	public static final VisualAdapterLinearRegression INSTANCE = new VisualAdapterLinearRegression();
	...
	public VisualizationModel toVisualModel(AnalyticOutPut analyzerOutPut,Locale locale)
		...
		if(analyzerOutPut instanceof AnalyzerOutPutTrainModel){
			...
			if(model.getModel() instanceof LinearRegressionGroupGPModel){
                LinearRegressionGroupGPModel linearRegressionGroupGPModel = (LinearRegressionGroupGPModel) model.getModel();
                Map<String,LinearRegressionModelDB> linearRegressionModelDBMap = linearRegressionGroupGPModel.getModelList();
                if(null!=linearRegressionModelDBMap){
                    //get run user
                    //String currentUser = getCurrentUser(analyzerOutPut);
                    String currentUser = (String) AlpineUtil.VALUE_PASSER.get();

                    linearRegressionGroupByModelMap.put(currentUser, null);
                    Set<Map.Entry<String,LinearRegressionModelDB>> linearRegressionModelDBEntrySet= linearRegressionModelDBMap.entrySet();
                    List<LinearRegressionModelDB> linearRegressionModelDBs = new ArrayList<LinearRegressionModelDB>();
                    List<String> keys = new ArrayList<String>();
                    if(null!=linearRegressionModelDBEntrySet){
                        Iterator<Map.Entry<String,LinearRegressionModelDB>> itor = linearRegressionModelDBEntrySet.iterator();
                        while (itor.hasNext()){
                             Map.Entry<String,LinearRegressionModelDB> entry = itor.next();
                            linearRegressionModelDBs.add(entry.getValue());
                            keys.add(entry.getKey());
                        }
                        DataTable dataTable4Text = getVtext4Table(linearRegressionModelDBs,keys,locale) ;
                        ...
                        tableModel.setVisualizationType(VisualAdapterLinearRegression.FOR_GROUP_BY4_LINEAR_TEXT_TABLE_TYPE);
                        models.add(tableModel);

                        if(linearRegressionModelDBs.size()>0){
                            Map<String,VisualizationModelComposite> modelCompositeMap = new HashMap<String, VisualizationModelComposite>();
                            for(int i=0;i<linearRegressionModelDBs.size();i++){
                                List<VisualizationModel> models4GroupBy= new ArrayList<VisualizationModel>();
                                LinearRegressionModelDB lr = linearRegressionModelDBs.get(i);
                                String key = keys.get(i);
                                buildGruopByModuls(models4GroupBy,lr,key,locale);
                                VisualizationModelComposite  visualModel= new VisualizationModelComposite(VisualNLS.getMessage(VisualNLS.DATA_GROUP,locale)+":'"+keys.get(i)+"'"	,models4GroupBy);
                                modelCompositeMap.put(key,visualModel);
                           }
                            linearRegressionGroupByModelMap.put(currentUser, modelCompositeMap);
                        }
                    }
                }

            }else{
                String message =   getVtextText((LinearRegressionModelDB)model.getModel()).toString();
                ...
                DataTable dataTable=getDataTable((LinearRegressionModelDB)model.getModel());
                VisualizationModelDataTable  tableModel=new VisualizationModelDataTable(
                        VisualNLS.getMessage(VisualNLS.DATA_TITLE,locale), dataTable);
                models.add( tableModel);
                //
                LinearRegressionModelDB linearDbModel = (LinearRegressionModelDB)model.getModel();
    ...
    private void buildGruopByModuls(List<VisualizationModel> models,LinearRegressionModelDB linearRegressionModelDB,String keyValue,Locale locale){
        DataTable dataTable=getDataTable(linearRegressionModelDB);
        VisualizationModelDataTable  tableModel=new VisualizationModelDataTable(
                VisualNLS.getMessage(VisualNLS.DATA_TITLE,locale), dataTable);
        models.add( tableModel);
        //
        LinearRegressionModelDB linearDbModel = linearRegressionModelDB;
        VisualizationModelScatter residualChartModel = new VisualizationModelScatter(
                VisualLanguagePack.getMessage(VisualLanguagePack.RESIDUALPLOT_TITLE,
                        locale), null);
        VisualizationModelScatter normalProbabilityPlotModel = new VisualizationModelScatter(
    ...
    private DataTable getVtext4Table(List<LinearRegressionModelDB> linearRegressionModelDBs, List<String> keys, Locale locale) {
        ...
        if(null!=linearRegressionModelDBs && linearRegressionModelDBs.size()>0){
           Iterator<LinearRegressionModelDB> itor = linearRegressionModelDBs.iterator();
            int i=0;
            while (itor.hasNext()){
                LinearRegressionModelDB modelDB= itor.next();
    ...
    private void fillResidualNormalProbablityChartModel(
			...
			LinearRegressionModelDB model, Locale locale)
...
//			scatterModel.setVisualizationType(VisualizationModel.TYPE_SCATTER_CHART);
	...
	private void fillResidualChartModel(VisualizationModel residualChartModel,
		...
		if(null!=model && model.getResiduals()!=null && model.getResiduals().size()>0){
            ...
            if(title4Precision[0]!=null && title4Precision[0].trim()!=""){
                String tempValue = title4Precision[0].trim().replaceAll("\"","").replace("(","").replace(")","");
                try{
                    xPrecision = Float.parseFloat(tempValue);
                }catch (Exception e){
                    xPrecision = 1.0f;
                }
            }
            VisualLine line = new VisualLine(points.getLabel());
            line.addVisualPoint(new VisualPoint(String.valueOf(maxMin.getMinX()/xPrecision),"0"));
	...
	private DataTable getDataTable(LinearRegressionModelDB model) {
		DataTable table= new DataTable();
		...
		for (int i = 0; i < coefficients.length; i++) {
            ...
            if("intercept".equals(coefficientsNames[i])==true){
