define([
...
], function(dojo, declare, connect, array, lang, html, json, win, query, keys, Source, Avatar, _Plugin, EnhancedGrid, OpUtil){
    ...
    var GridDnDElement = declare("alpine.layout.GridDnDElement", null, {
    ...
    var GridDnDAvatar = declare("alpine.layout.GridDnDAvatar", Avatar, {
        constructor: function(manager,operatorKey){
        ...
        construct: function(){
            if (this.operatorKey)
            {
                var operator = OpUtil.getOperatorObjectByKey(this.operatorKey);
            }

            if (operator)
            {
                var a = html.create("img", {"src": OpUtil.getImageSourceByImageType(operator.imgtype),
                    "class": "dojoxGridDndAvatar",
                    "style": {
                        position: "absolute",
                        zIndex: "1999",
                        margin: "0px",
                        boxShadow: "0px 0px 0px rgba(255,0,0,0.25)",
                        border:"0px",
            ...
            html.create("span", {
                ...
                "innerHTML": source.generateText ? this._generateText() : "la la la"
            }, td);
    ...
    var OperatorDnd = declare("alpine.layout.OperatorDnd", _Plugin, {
        // summary:
        ...
        _createSource: function(evt){
            ...
            var object = evt.grid._by_idx[rowindex].item;
            console.log("the operatorclass: " + object.operatorClass);
            if (object.operatorClass == "DbTableOperator")
            {
                var operatorkey = "DbTableOperator";

            }   else if (object.operatorClass == "HadoopFileOperator")
            {
                var operatorkey = "HadoopFileOperator";
            }  else
            {
                var operatorkey = evt.grid._by_idx[rowindex].idty;
            }
            console.log("you have selected: " + operatorkey);
            var m = dojo.dnd.manager();
            var oldMakeAvatar = m.makeAvatar;
            m._dndPlugin = this;
            m.makeAvatar = function(){
                var avatar = new GridDnDAvatar(m, operatorkey);
                delete m._dndPlugin;
                return avatar;
            };
    ...
    EnhancedGrid.registerPlugin(OperatorDnd/*name:'dndop'*/, {
