dojo.declare("httpService",null,{
    ...
    success:function (text, args, callbackpanelid, callback, errorcallback, logincallback) {
        if (callbackpanelid) alpine.spinner.hideSpinner(callbackpanelid);
        ...
        if (text && text != "") {
            ...
            if (obj.error_code == -1) {
                ...
                popupComponent.alert(obj.message == null ? alpine.nls.message_unknow_error : obj.message, null, function () {
    ...
    //called before get/post/postWithLogin/upload - currently starts spinner
    startCall:function(callbackpanelid, url)
    {
        if (callbackpanelid)
        {
            console.log("has a spinner id:" + callbackpanelid);
            alpine.spinner.showSpinner(callbackpanelid);
        }  else
        {
            console.log("no spinner id for get url " + url);
        }
    },

        //add errorcallback to make it more user-friendly
	get : function(url, callback, errorcallback, isSync, callbackpanelid){
        this.startCall(callbackpanelid, url);
	...
	upload : function(url, form_name, callback, errorcallback, callbackpanelid, contentfnx){
        ...
        {
            console.log("no spinner id for upload url " + url);
        }
        dojo.io.iframe.send( {
			url : encodeURI(url),
			form : form_name,
