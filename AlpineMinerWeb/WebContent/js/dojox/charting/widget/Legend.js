			...
			if(dyn.fill){
				...
				if(dyn.marker){
					...
					if(dyn.stroke){
						...
						console.log("dyn.color--->" + dyn.color);
						console.log("dyn.markerFill--->" + dyn.markerFill);
						console.log("dyn.markerStroke.color--->" + dyn.markerStroke.color);
						surface.createPath({path: "M" + c.x + " " + c.y + " " + dyn.marker}).
							setFill(dyn.color ? dyn.color : dyn.markerFill).setStroke(dyn.color ? dyn.color : dyn.markerFill);
						//fixed no legend icon appeared if only one point in series. by Gary
//						setFill(dyn.markerFill).setStroke(dyn.markerStroke.color);
					}
				}
			}else{
				// nothing
