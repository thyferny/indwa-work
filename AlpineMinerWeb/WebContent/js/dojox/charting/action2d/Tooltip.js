define(["dojo/_base/kernel", "dijit/Tooltip","dojo/_base/lang", "dojo/_base/html", "dojo/_base/declare", "./PlotAction", 
	...
	return declare("dojox.charting.action2d.Tooltip", PlotAction, {
		...
		process: function(o){
			...
			//aroundRect.x += lt.x;
			//aroundRect.y += lt.y;
			//**********fixed Tooltip visualize weird if scroll screen. by Gary************
			aroundRect.x -= this.chart.node.offsetParent.scrollLeft;
			aroundRect.y  -= this.chart.node.offsetParent.scrollTop;
			// lt.x will be less than zero, if scrolled screen.
			if(lt.x > 0){
				aroundRect.x += lt.x;
			}
			if(lt.y > 0){
			...
			aroundRect.w = Math.ceil(aroundRect.w ? aroundRect.w : aroundRect.width);// fixed js error if bar is beside the edge of screen. by Gary
			aroundRect.h = Math.ceil(aroundRect.h ? aroundRect.h : aroundRect.height);// fixed js error if bar is beside the edge of screen. by Gary
