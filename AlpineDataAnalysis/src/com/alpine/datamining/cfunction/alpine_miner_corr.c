...
PG_FUNCTION_INFO_V1(alpine_miner_corr_accum);
PG_FUNCTION_INFO_V1(alpine_miner_corr_combine);
PG_FUNCTION_INFO_V1(alpine_miner_corr_final);

Datum
alpine_miner_corr_accum(PG_FUNCTION_ARGS)
{
...
Datum
alpine_miner_corr_combine(PG_FUNCTION_ARGS)
{
...
Datum
alpine_miner_corr_final(PG_FUNCTION_ARGS){
		if (PG_ARGISNULL(0))
		PG_RETURN_NULL();
		ArrayType *state = PG_GETARG_ARRAYTYPE_P(0);
        	    ArrayType    *vari;
		...
		int varisize = column_size * sizeof(float8) + ARR_OVERHEAD_NONULLS(1);
	
		vari =(ArrayType *) palloc(varisize);
		SET_VARSIZE(vari , varisize);
		vari ->ndim=1;
		vari ->dataoffset = 0;
		...
		for ( i = 0; i < column_size; i++){
				for( j = i; j < column_size; j++){
		         	...
		         	resultData[k] =state_array_data[k]/row_size- state_array_data[result_size+i] * state_array_data[result_size+j]/row_size/row_size;
					if(i==j){
						variData[i]=sqrt(resultData[k]);
		...
		for ( i = 0; i < column_size; i++){
				for( j = i; j < column_size; j++){
						...
						resultData[k] =resultData[k]/(variData[i]*variData[j]);
