 ...
 */
static float8 alpine_miner_svm_dot(ArrayType * x1, ArrayType * x2)
{
	int dim1;
	int dim2;
	float8 * fx1;
	...
	float8 ret;
	int i;
	if (ARR_NDIM(x1) != 1 || ARR_ELEMTYPE(x1) != FLOAT8OID ||
	    ARR_NDIM(x2) != 1 || ARR_ELEMTYPE(x2) != FLOAT8OID)
		ereport(ERROR,
		       (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
			errmsg("function alpine_miner_svm_dot called with invalid parameters"
			       )));
	...
	fx1 = (float8 *)ARR_DATA_PTR(x1);
	fx2 = (float8 *)ARR_DATA_PTR(x2);
 ...
 */
static float8 alpine_miner_svm_polynomial(ArrayType * x1, ArrayType * x2, int degree)
{
	int dim1;
	int dim2;
	float8 * fx1;
	...
	if (ARR_NDIM(x1) != 1 || ARR_ELEMTYPE(x1) != FLOAT8OID ||
		...
		ereport(ERROR,
		       (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
			errmsg("function alpine_miner_svm_polynomial called with invalid parameters"
			       )));
 ...
 * This function is the polynomial kernel
 */
float8 alpine_miner_svm_gaussian(ArrayType * x1, ArrayType * x2, float8 gamma)
{
	...
	if (ARR_NDIM(x1) != 1 || ARR_ELEMTYPE(x1) != FLOAT8OID ||
		...
		ereport(ERROR,
		       (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
			errmsg("function alpine_miner_svm_gaussian called with invalid parameters"
			       )));
 ...
 */
static float8 apply_kernel(int kernel_type, int degree, double gamma, ArrayType * x1, ArrayType * x2) 
{
	if (kernel_type == 1)
	{
		return alpine_miner_svm_dot(x1, x2);
	}
	else if (kernel_type == 2)
	{
		return alpine_miner_svm_polynomial(x1, x2, degree);
	}
	else
	{
		return alpine_miner_svm_gaussian(x1, x2, gamma);
	}
}
 ...
 */
static float8 
alpine_miner_svm_predict_eval(int kernel_type, int degree, double gamma, float8 * weights, ArrayType * supp_vectors, 
		 ArrayType * ind, int32 nsvs, int32 ind_dim)
{
	...
	for (i=0; i!=nsvs; i++) {
		...
		ret += weights[i] * apply_kernel(kernel_type, degree, gamma, spp_vec, ind);
	}
	return ret;
}

Datum alpine_miner_svm_predict_sub(PG_FUNCTION_ARGS);
PG_FUNCTION_INFO_V1(alpine_miner_svm_predict_sub);
...
//	text * kernel = PG_GETARG_TEXT_P(5);
	int  kernel_type = PG_GETARG_INT32(5);
	int degree = PG_GETARG_INT32(6);
	float8 gamma = PG_GETARG_FLOAT8(7);
...
//	Oid koid = LookupFuncName(funcname, 2, argtypes, false);
	...
	ret = alpine_miner_svm_predict_eval( kernel_type, degree, gamma,weights,supp_vecs_arr,ind_arr,nsvs,ind_dim);
...
Datum alpine_miner_svm_reg_update(PG_FUNCTION_ARGS);
PG_FUNCTION_INFO_V1(alpine_miner_svm_reg_update);
...
//	text * kernel = PG_GETARG_TEXT_P(3);
	...
	int  kernel_type = PG_GETARG_INT32(6);
	int degree = PG_GETARG_INT32(7);
	float8 gamma = PG_GETARG_FLOAT8(8);
	float8 * ind ;
	...
	inds = DatumGetInt32(GetAttributeByName(t, "inds", &nil[0]));
	cum_err =DatumGetFloat8(GetAttributeByName(t,"cum_err",&nil[1]));
	epsilon =DatumGetFloat8(GetAttributeByName(t,"epsilon",&nil[2]));
	rho = DatumGetFloat8(GetAttributeByName(t, "rho", &nil[3]));
	b = DatumGetFloat8(GetAttributeByName(t, "b", &nil[4]));
	nsvs = DatumGetInt32(GetAttributeByName(t, "nsvs", &nil[5]));
	ind_dim =DatumGetInt32(GetAttributeByName(t, "ind_dim", &nil[6]));
	weights_arr = 
...
//	}
	...
	p = alpine_miner_svm_predict_eval(kernel_type, degree, gamma,weights,supp_vecs_arr,ind_arr,nsvs,ind_dim); 
...
Datum alpine_miner_svm_cls_update(PG_FUNCTION_ARGS);
PG_FUNCTION_INFO_V1(alpine_miner_svm_cls_update);
...
//	}
	...
	p = alpine_miner_svm_predict_eval(kernel_type, degree, gamma,weights,supp_vecs_arr,ind_arr,nsvs,ind_dim) + b; 
	p = label * p;
...
Datum alpine_miner_svm_nd_update(PG_FUNCTION_ARGS);
PG_FUNCTION_INFO_V1(alpine_miner_svm_nd_update);
...
//	text * kernel = PG_GETARG_TEXT_P(2);
	...
	float8 nu = PG_GETARG_FLOAT8(3);  /* compression parameter; about nu
					   ...
					   * become support vectors
					   */
	int  kernel_type = PG_GETARG_INT32(4);
	int degree = PG_GETARG_INT32(5);
	float8 gamma = PG_GETARG_FLOAT8(6);
	float8 * ind;
	bool nil[9] = { 0,0,0,0,0,0,0,0,0};
