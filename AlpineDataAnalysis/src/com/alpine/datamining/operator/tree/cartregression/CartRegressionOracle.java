...
public class CartRegressionOracle implements INumericalSql {

	public String getAvgString(String labelColumnName, String columnName, String tableName, String whereCondition){
		String sql = "";
		sql = (" select ")+(labelColumnName)+(",  avg(")+(columnName)+(")  over ( order by ");
	...
	public StringBuilder getVarCountSql(String whereCondition,
		...
		sql.append("variance(1.0*("+("case when "+whereEqual+(" then ")+(labelColumnName)+(" else null end"))).append(")),");
	...
	public StringBuffer getVarianceSql(String labelColumnNameWithCast,
		...
		sql.append("select case when count > 1 then variance else null end variance from (");
		sql.append("select count(*) as count, variance(").append((labelColumnNameWithCast)).append(") as variance from ");
	...
	public StringBuffer getNumericSplitSql(String labelColumnNameWithCast,
		...
		sql.append(" from (").append("select avg,lessavg,moreavg").append(",(case when lesscount > 1 then lessvariance else null end) lessvariance");
