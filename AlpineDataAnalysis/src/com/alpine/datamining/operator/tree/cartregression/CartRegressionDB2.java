...
public class CartRegressionDB2 implements INumericalSql {

	public String getAvgString(String labelColumnName, String columnName, String tableName, String whereCondition){
		String sql = "";
		sql = (" select ")+(labelColumnName)+(",  avg(double(")+(columnName)+("))  over ( order by ");
	...
	public StringBuilder getVarCountSql(String whereCondition,
		...
		sql.append("  from ( ");
		sql.append("select ");
		sql.append("count(").append(columnName).append(") as count ,variance(double(").append(labelColumnName);
		sql.append(")) as var ,sum(case when ").append(whereEqual).append(" then 1 else 0 end) as countequal ,");
	...
	public String getChangeToLeafSql(String selectSQL, String labelColumnName) {
		String sql = "select avg, case when count > 1 then variance/(count - 1 ) * count else null end as variance, count from (select avg(double("+labelColumnName+")) as avg ,variance(double("+labelColumnName+")) as variance ,count("+labelColumnName+") as count from ("+selectSQL+") foo where "+labelColumnName+" is not null) foo";
		return sql;
	}
