...
public class CartClassificationGP implements ICartClassfication {
	...
	public String generateNumericSql(Column labelColumn,
		...
		if(distinctRatio < distinctRatioThreshold)
		{
			...
			sql.append(" rows between 0 preceding and 1 following ) as avg ,count").append(
					" from ( ");
			sql.append("select ").append(columnName);
			sql.append(",").append(count);
			sql.append(" from  ").append(tableName).append(" ").append(whereCondition).append(" group by ").append(columnName).append(" ) as foo1 ) as foo2)  as foo3 )");
		}
		else
		{
			...
			sql
					.append(
							" rows between 0 preceding and 1 following ) as avg from  ").append(tableName).append(" ").append(whereCondition).append(" ) as foo1 group by avg) as foo2)  as foo3 left join ( select ").append(columnName)
