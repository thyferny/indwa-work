...
public class MultiDBSqlPostgres implements MultiDBSql {
	...
	public String generateNumericSql(Column labelColumn,
        ...
        sql.append("select ").append(avgAlias).append(",").append(count).append(" from (");

        sql.append("select ").append(labelColumnName).append(",");
        sql.append(" (case when alpineleadvalue is not null then (alpineleadvalue::float+ ").append(columnName).append(")/2.0 else ").append(columnName).append(" end) as ").append(avgAlias);
        sql.append(" from (select ").append(labelColumnName).append(",");
        ...
        sql.append(" from ( ").append(selectSQL).append(") as foo0)");
