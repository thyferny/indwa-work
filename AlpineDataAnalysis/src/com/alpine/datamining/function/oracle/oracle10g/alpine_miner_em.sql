...
CREATE FUNCTION ALPINE_MINER_EM_TRAIN(tablename     varchar2,
...
begin
  ...
  sqlexecute := 'create table ' || tempbytemp || '    as ( select  ' ||
                tablename || '.*';
  for i in 1 .. clusternumber loop
    sqlexecute := sqlexecute || ',alpine_miner_em_getp(Varchar2Array(';
    sqlexecute := sqlexecute ||array_to_string(columnname, ',');
    sqlexecute := sqlexecute || '),floatarray(';
    sqlexecute := sqlexecute || GETFARANGE(mu, (i - 1) * columnsize + 1, (i * columnsize));
    sqlexecute := sqlexecute ||'),floatarray(';
    sqlexecute := sqlexecute || GETFARANGE(sigma,(i - 1) * columnsize + 1,(i * columnsize)) ;
    sqlexecute := sqlexecute|| '),' ;
    
    sqlexecute := sqlexecute|| alpha(i) ;
    sqlexecute := sqlexecute||') as alpine_miner_em_p' || i;
  end loop;
  ...
  sqlexecute := 'create table ' || temptable || '   as (select ' ||
      ...
      if i = 1 then
        ...
        for j in 1 .. columnsize loop
          if j = 1 then
            ...
            tempmu := tempmu || ',sum(' || columnname(j) ||
                      '*alpine_miner_em_p' || i ||
                      '/alpine_miner_em_sum)/sum(alpine_miner_em_p' || i ||
                      '/alpine_miner_em_sum)';
          end if;
        ...
        tempalpha := tempalpha || ',avg(alpine_miner_em_p' || i ||
                     '/alpine_miner_em_sum)';
        for j in 1 .. columnsize loop
          tempmu := tempmu || ',sum(' || columnname(j) ||
                    '*alpine_miner_em_p' || i ||
      ...
      for j in 1 .. columnsize loop
        if i = 1 then
          if j = 1 then
            ...
            tempsigma := tempsigma || ',sum((' || columnname(j) || '- ' ||
                         mu(k) || ')*(' || columnname(j) || '- ' || mu(k) ||
                         ')*alpine_miner_em_p' || i ||
                         '/alpine_miner_em_sum)/sum(alpine_miner_em_p' || i ||
                         '/alpine_miner_em_sum)';
