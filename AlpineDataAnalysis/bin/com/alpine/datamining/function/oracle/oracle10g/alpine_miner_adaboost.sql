...
BEGIN
  ...
  dbms_lob.append(sqlan, ')');
  execute immediate to_char(sqlan);
  execute immediate to_char(sqlan1);
  commit;
  execute immediate 'alter session force parallel dml';
...
BEGIN
  ...
  execute immediate 'select count(*) from ' || schemaname || '.' ||
                    tablename
    into rownumber;
  execute immediate ' select max("alpine_adaboost_totalpeoso")  from ' ||
  ...
  execute immediate 'create table ' || schemaname || '."r' || stamp ||
                    ...
                    ')  "alpine_miner_adaboost_r" from ' || schemaname || '.' ||
  ...
  if partnumber = 1 then
    ...
    execute immediate 'create table ' || schemaname || '."s' || stamp ||
                      ...
                      '."r' || stamp || '"."alpine_miner_adaboost_r" and ' ||
                      schemaname || '.' || tablename ||
                      '."alpine_adaboost_peoso" > (' || schemaname || '.' ||
                      tablename || '."alpine_adaboost_totalpeoso"-' ||
                      schemaname || '."r' || stamp ||
                      '"."alpine_miner_adaboost_r")';
    executesql := 'alter session disable parallel dml';
    execute immediate executesql;
    commit;
  else
    tempstring := ' select "alpine_adaboost_totalpeoso" as peoso from ' ||
                  schemaname || '.' || tablename ||
                  ' where mod("alpine_adaboost_id",' || partsize ||
                  ')=0 order by peoso';
    i          := 1;
    splitpeoso.extend();
    splitpeoso(i) := 0;
    ...
    tempstring := 'create table ' || schemaname || '."s' || stamp ||
                  ...
                  schemaname || '.' || tablename ||
                  ' where "alpine_adaboost_totalpeoso">' || splitpeoso(i) ||
                  ' and  "alpine_adaboost_totalpeoso"<=' ||
                  splitpeoso(i + 1) || ')   foo' || i ||
                  ' join (select * from "r' || stamp ||
                  '" where "alpine_miner_adaboost_r" >' || splitpeoso(i) ||
                  ' and  "alpine_miner_adaboost_r"<=' || splitpeoso(i + 1) ||
                  ')   foor' || i || ' on foo' || i ||
                  '."alpine_adaboost_totalpeoso" >=foor' || i ||
                  '."alpine_miner_adaboost_r" and foo' || i ||
                  '."alpine_adaboost_peoso" > (foo' || i ||
                  '."alpine_adaboost_totalpeoso"-foor' || i ||
                  '."alpine_miner_adaboost_r") ';
    ...
    for i in 2 .. partnumber loop
      tempstring := '  insert into  ' || schemaname || '."s' || stamp ||
                    ...
                    tablename || ' where "alpine_adaboost_totalpeoso">' ||
                    splitpeoso(i) || ' and  "alpine_adaboost_totalpeoso"<=' ||
                    splitpeoso(i + 1) || ')   foo' || i ||
                    ' join (select * from "r' || stamp ||
                    '" where "alpine_miner_adaboost_r" >' || splitpeoso(i) ||
                    ' and "alpine_miner_adaboost_r"<=' || splitpeoso(i + 1) ||
