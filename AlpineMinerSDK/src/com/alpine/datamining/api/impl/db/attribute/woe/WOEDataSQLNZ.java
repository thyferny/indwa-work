 ...
 *
 */
public class WOEDataSQLNZ extends WOEDataSQL{
    private static final Logger itsLogger = Logger.getLogger(WOEDataSQLNZ.class);
    static {
		DBType=DataSourceInfoNZ.dBType;
		}
	...
	public void generalAfterComputeWOESQL(String inputSchema, String tableName,
		...
		sqlBuffer.append("   from ").append(StringHandler.doubleQ(inputSchema))
				.append(".").append(StringHandler.doubleQ(tableName)).append(
						") as foo group by \"AlpineWOEGroup\"");
	}
	...
	public AnalysisWOETable createTable(AnalyticSource analyticSource,
		...
		if (dropIfExist.equalsIgnoreCase(Resources.YesOpt)) {
			...
			String sql = "call droptable_if_existsdoubleq('"
					+ StringHandler.doubleQ(outputTableName) + "')";
			itsLogger.debug("WOEDataSQLNZ.createTable():sql=" + sql);
			st.execute(sql);

		}
		createBuffer.append(selectSQL) ;
		itsLogger.debug("WOEDataSQLNZ.createTable():sql=" + createBuffer.toString());
