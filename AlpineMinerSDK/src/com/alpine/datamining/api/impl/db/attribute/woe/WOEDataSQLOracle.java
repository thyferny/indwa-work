/**
* ClassName WOEDataSQLOracle.java
*
 ...
 *
 */
public class WOEDataSQLOracle extends WOEDataSQL{
    private static final Logger itsLogger = Logger.getLogger(WOEDataSQLOracle.class);

    static {
		DBType=DataSourceInfoOracle.dBType;
		}
	/* (non-Javadoc)
	 * @see com.alpine.datamining.api.impl.db.attribute.WOE.WOEDataSQL#getComputeWOESQL(java.lang.String, java.lang.String, java.lang.StringBuffer)
	 */
	@Override
	public void generalPreComputeWOESQL(String dependColumn, String goodValue,
		...
		sqlBuffer.append(" select ").append("  1.0*sum( case when  ").append(
				...
				StringHandler.doubleQ(dependColumn)).append("  = '").append(
				goodValue).append("' then 0  else 1  end) \"ALPINEWOEBAD\" ,\"AlpineWOEGroup\" ")
				.append("from (select ").append(StringHandler.doubleQ(dependColumn)) ;
	}
	...
	public void generalAfterComputeWOESQL(String inputSchema, String tableName,
			...
			.append(") group by \"AlpineWOEGroup\"");
	}
	...
	public  AnalysisWOETable createTable(AnalyticSource analyticSource,
		...
		sqlBuffer.append(" create table  ").append(
				...
				StringHandler.doubleQ(outputTableName)).append(
				" as select ").append(StringHandler.doubleQ(inputSchema))
		...
		if (dropIfExist.equalsIgnoreCase(Resources.YesOpt)) {
				...
				itsLogger.debug("WOEDataSQLOracle.createTable():sql=" + sql);
				st.execute(sql);
			
		}
		itsLogger.debug("WOEDataSQLOracle.createTable():sql=" + sqlBuffer.toString());
