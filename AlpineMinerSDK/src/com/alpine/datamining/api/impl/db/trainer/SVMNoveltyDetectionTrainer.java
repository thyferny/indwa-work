/**
 * ClassName SVMNoveltyDetectionTrainer
 *
...
import com.alpine.datamining.api.impl.algoconf.SVMNoveltyDetectionConfig;
...
import com.alpine.datamining.operator.svm.SVMNoveltyDetection;
...
public class SVMNoveltyDetectionTrainer extends AbstractDBModelTrainer {
	private static Logger logger= Logger.getLogger(SVMNoveltyDetectionTrainer.class);
	
	protected   Model train( AnalyticSource source )
			throws AnalysisException  {

		try {
			SVMNoveltyDetectionConfig config=(SVMNoveltyDetectionConfig)source.getAnalyticConfig(); 
			...
			Operator learner = OperatorUtil.createOperator(SVMNoveltyDetection.class);
	...
	protected AnalyticNodeMetaInfo createNodeMetaInfo(Locale locale) {
		...
		nodeMetaInfo.setAlgorithmName(SDKLanguagePack.getMessage(SDKLanguagePack.SVM_ND_TRAIN_NAME,locale));
		nodeMetaInfo.setAlgorithmDescription(SDKLanguagePack.getMessage(SDKLanguagePack.SVM_ND_TRAIN_DESCRIPTION,locale));
