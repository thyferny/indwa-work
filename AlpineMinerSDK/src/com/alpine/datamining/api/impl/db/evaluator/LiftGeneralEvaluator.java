/**
 * ClassName LiftGeneralEvaluator.java
 *
...
import com.alpine.datamining.operator.evaluator.LiftDataGeneratorGeneral;
import com.alpine.datamining.utility.OperatorUtil;

/**
 *
 */
public class LiftGeneralEvaluator extends AbstractDBModelValidator {
	private static Logger logger= Logger.getLogger(LiftGeneralEvaluator.class);
...
//			dataSet.recalculateAllcolumnStatistics();
			...
			{
//				operator.setParameter(LiftDataGeneratorGeneral.PARAMETER_USE_MODEL, config.getUseModel());
				parameter.setUseModel(Boolean.parseBoolean(config.getUseModel()));
			}
			if (config.getColumnValue() != null)
			{
//				operator.setParameter(LiftDataGeneratorGeneral.PARAMETER_TARGET_CLASS, config.getColumnValue());
